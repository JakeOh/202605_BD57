...
--> 오라클에서는 문자열 '1982/01/01'을 날짜(DATE) 타입으로 변환 후 hiredate 컬럼 값과 크기를 비교.

-- select * from emp where hiredate > '이것은 날짜가 아님';
-- 위와 같은 문장을 실행하면 에러가 발생함.
-- 그 이유는 문자열 '이것은 날짜가 아님'이 날짜 타입으로 변환될 수 없는 문자열이기 때문에.

-- 암묵적(암시적) 타입 변환: 컬럼의 데이터와 값을 비교하기 위해서 오라클에서 비교하려는 값을 자동으로 변환하는 것.
-- 명시적 타입 변환: 비교하려는 값을 to_date, to_number, to_char와 같은 함수를 사용해서 직접 타입을 변환하는 것.
